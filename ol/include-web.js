...
(function () {
    var r = new RegExp("(^|(.*?\\/))(include-web\.js)(\\?|$)"),
        s = document.getElementsByTagName('script'), targetScript,workPath;
    for (var i = 0; i < s.length; i++) {
        ...
        if (src) {
            var m = src.match(r);
            if (m) {
				workPath=src.replace("include-web.js","dist/");
                targetScript = s[i];
                break;
            }
        }
    ...
    function load() {
        ...
        inputScript(workPath+"tokengenerator.js");
        ...
        if (!inArray(excludes, 'example-i18n')) {
            ...
            inputScript(workPath+"utils.js");
            inputScript(workPath+"localization.js");
        ...
        if (inArray(includes, 'widgets.alert')) {
            inputScript(workPath+"widgets.js");
        }
