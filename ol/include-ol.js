(function () {
    var r = new RegExp("(^|(.*?\\/))(include-ol\.js)(\\?|$)"),
        s = document.getElementsByTagName('script'),workPath,
        targetScript;
    for (var i = 0; i < s.length; i++) {
        ...
        if (src) {
            var m = src.match(r);
            if (m) {
				workPath=src.replace("include-ol.js","dist/");
                targetScript = s[i];
                break;
            }
        }
    ...
    function load() {
        ...
        if (!inArray(excludes, 'iclient-ol')) {
          if (supportES6()) {
              inputScript(workPath+"iclient-ol-es6.min.js");
          } else {
              inputScript(workPath+"iclient-ol.min.js");
          }
        }
        if (!inArray(excludes, 'iclient-ol-css')) {
            inputCSS(workPath+"iclient-ol.min.css");
        }
    }
