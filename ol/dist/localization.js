...
(function (nameSpace, $) {
    var targetScript = (function () {
        var r = new RegExp("(^|(.*?\\/))(localization\.js)(\\?|$)"),
            s = document.getElementsByTagName('script');
        for (var i = 0; i < s.length; i++) {
            var src = s[i].getAttribute('src');
            if (src && src.match(r)) {
				workPath=src.replace("localization.js","");
                return s[i];
            }
        }
    })();
	...
	Localization.initializeI18N(workPath, function () {Localization.localize();Localization.initGlobal();})
})(Localization, jQuery);
